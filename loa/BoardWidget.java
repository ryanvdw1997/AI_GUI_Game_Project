...
class BoardWidget extends Pad {
    ...
    /** Handle a mouse-button push on S FIX ME. */
    private void mousePressed(Square s) {
        pHolder = _board.get(s);
        holder = s;
    }

    /** Handle a mouse-button release on S FIX ME. */
    private void mouseReleased(Square s) {
        if (_board.isLegal(holder, s)) {
            _commands.offer(String.format("%s-%s", holder.toString(),
                    s.toString()));
        }
        repaint();
    }
    ...
    /** Revise the displayed board according to BOARD FIX ME. */
    ...
    /** Holder variable for the square pressed on in mousePressed. */
    private Square holder;

    /** Piece holder for the square holder variable. */
    private Piece pHolder;
